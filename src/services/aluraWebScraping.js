...
function getData(forumUrl, pagesToRead, titleSelector, courseNameSelector, forumName) {
    let currentPage = 1;
    const titles = [];
    const courses = [];
    ...
    const getForumData = setInterval(async () => {
        ...
        if (currentPage === pagesToRead) {
            const coursesOcurrences = getCourseOcurrencesInArray(courses).sort((a, b) => b.count - a.count);
            const coursesFiltered = courses.filter((course, index) => courses.indexOf(course) === index);
            const data = { coursesOcurrences, titles, coursesFiltered};
    
            //console.log(data);
            console.log("Total de dados: ", titles.length + coursesFiltered.length);
...
function saveData(jsonData, date, forumName) {
    const file = fs.writeFile(
        path.join("./src/data/forumDataOutput", `data-${forumName}-${date}.json`),
        jsonData,
        "utf8",
        (err) => {
            if (err) {
